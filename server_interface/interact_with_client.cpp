...
/* FULL FUNCTIONS DESCRIPTIONS IN interact_with_client.h */
using namespace std;
...
// Check if the target the client asked for is valid / exists
bool validateTarget(string& targetName, ClientInfo& client, list<ClientInfo>& all_clients)
{
	/* Note : We give 'serverReply' the prefix "S:" and later we give messages from
...
// Retreive the target again before messaging to ensure he did not disconnect
...
void Interact(SOCKET clientSocket, list<ClientInfo>& all_clients)
{
	...
	*/
	...
	while (true) {
		...
		message = string(buffer, bytesRecved); // Client's message
		if (isQuit(message, client)) // If client sent 'Quit' then quit the loop
			break;

		switch (mode) {
		case REGISTER:
			// create the client {name , socket}, Add them to the list and procced to SEARCH_TARGET
		...
		case SEARCH_TARGET: // Find the target our client tries to reach
			// Send feedback on search error/success and procced to CHAT if target was found 
		...
		case CHAT: // Message target client by forwarding ('send') client's message (client -> server (we're here) -> target)
			if (equalStrings(message, "BACK")) // If client wants a different target then return to SEARCH_TARGET
			{
				mode = SEARCH_TARGET;
				break;
			}
			ClientInfo* targetClient = getTarget(targetName, client, all_clients); // Check if target hasn't disconnected
			if (targetClient == NULL) { // If Target has disconnected, reach a different one
				mode = SEARCH_TARGET;
				break;
			}
