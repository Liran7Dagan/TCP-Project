...
public:
	...
	// A method to find a specific client in a list, We are given that all client names are unique by the project's demands
	...
	// Given a message, deliver it to this client via this socket with "send" function. true for a successful send, false for a failed send
	bool getMessage(const string& message);

	// Given a list containing this client, remove them from the list.
